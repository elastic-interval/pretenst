...
impl KleinFabric {
    ...
    fn random_joint(&mut self) -> UniqueId {
    ...
    fn push(&mut self, alpha: UniqueId, omega: UniqueId) -> UniqueId {
        self.fabric.create_interval(alpha, omega, PUSH, 1.0)
    }

    fn pull(&mut self, alpha: UniqueId, omega: UniqueId) -> UniqueId {
        self.fabric.create_interval(alpha, omega, PULL, 1.0)
    }
...
pub fn generate_klein(width: usize, height: usize, shift: usize) -> Fabric {
    ...
    let joint_ids: Vec<_> = (0..w * h / 2).map(|_| kf.random_joint()).collect();
    let joint = |x: isize, y: isize| {
        let flip = (y / h) % 2 == 1;
        let x_rel = if flip { sh - 1 - x } else { x };
        let x_mod = (w * 2 + x_rel) % w;
        let y_mod = y % h;
        joint_ids[((y_mod * w + x_mod) / 2) as usize]
    };
    for y in 0..h {
        for x in 0..w {
            if (x + y) % 2 == 0 {
